...
export function DataTablePagination<TData, TValue>({
  ...
  // Create query string
  const [pageIndexQueryValue, setPageIndexQueryValue] = useQueryState("page");
  const [pageSizeQueryValue, setPageSizeQueryValue] = useQueryState("limit");
    ...
    const elements = filters.map((filter) => {
      switch (filter.type) {
        case DataTableComponentType.Search:
          ...
          return (
            <DataTableSearch {...(filter.props as DataTableSearchProps)} />
          );
      ...
      <div className="flex flex-col items-center justify-end gap-2 space-x-2 py-4 sm:flex-row">
        <div className="flex w-full items-center justify-between">
          <div className="flex-1 text-sm text-muted-foreground">
            {totalItems > 0 ? (
              <>
                Hiển thị {pageIndex * pageSize + 1} tới{" "}
                {Math.min((pageIndex + 1) * pageSize, totalItems)} của{" "}
                {totalItems} kết quả
              </>
            ) : (
              "Không tìm thấy kết quả"
